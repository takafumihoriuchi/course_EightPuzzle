...
#include <stdlib.h>
#include <time.h>
#include "eight_puzzle_player.h"

int main(void)
{
	int map[3][3];
	init_map(map);
	while (!game_completed(map)) {
		print_map(map);
		player_input(map);
	}
	print_map(map);
...
// returns 1 if puzzle completed, 0 if not
int game_completed(int map[][3])
{
...
void operate(int map[][3], char direction)
{
	...
	zero_row = find_zero_row(map);
	zero_col = find_zero_col(map);
	switch (direction) {
		case 'e' :
...
int validity(int map[][3], char direction)
{
	int zero_row, zero_col;
	zero_row = find_zero_row(map);
	zero_col = find_zero_col(map);
...
void player_input(int map[][3])
{
	...
	while (1) {
		...
		if (move_zero_dir=='e' || move_zero_dir=='d' || 
			...
			valid = validity(map, move_zero_dir);
			if (valid) {
